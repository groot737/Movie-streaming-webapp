...
const fetchKlipySearch = async (query, signal) => {
  ...
  const res = await fetch(
    KLIPY_API_BASE +
      "/" +
      encodeURIComponent(KLIPY_API_KEY) +
      "/gifs/search?" +
      params.toString(),
    {
      signal,
...
function FeedPage() {
  ...
  useEffect(() => {
    ...
    const loadNews = async () => {
      ...
      setError("");
      try {
        const response = await fetch(API_BASE + "/api/news/movie", {
          credentials: "include",
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok) {
        ...
        const results = Array.isArray(data?.items)
          ...
          : Array.isArray(data?.results)
            ...
            : Array.isArray(data?.news)
              ...
              : [];
        setItems(results);
        const fetchedAt = parseFetchedAt(
          data?.fetched_at || data?.fetchedAt || data?.fetchedAtMs
        );
        setLastFetched(fetchedAt || null);
      } catch (err) {
        if (active) {
    ...
    const next = comments.map((comment) => {
      if (comment.id === targetId) {
        ...
        return {
          ...comment,
          liked,
          likes: typeof likeCount === "number" ? likeCount : comment.likes || 0,
        };
      }
      if (comment.replies?.length) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="sticky top-0 z-20 bg-slate-950/90 backdrop-blur border-b border-slate-900">
        <div className="max-w-3xl mx-auto flex flex-wrap justify-center gap-2 sm:gap-3 py-4 sm:py-3 px-4">
          {[
            { id: "news", label: "News" },
        ...
        {activeTab === "news" && (
          <section className="space-y-6">
            <div className="flex flex-wrap items-end justify-between gap-2">
              <div>
                <h1 className="text-lg sm:text-xl font-semibold">Movie news</h1>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  Fresh updates from across the entertainment world.
                </p>
              </div>
              {lastFetched ? (
                <div className="text-[11px] text-slate-500">
                  Last updated{" "}
                  {new Date(lastFetched).toLocaleString(undefined, {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {items.map((item) => {
                  ...
                  const sourceName =
                    item.source_name || item.source_id || "Source";
                  const articleTime = item.pubDate || item.pubDateTZ || "";
                  const categoryLabel = Array.isArray(item.category)
                    ? item.category[0]
                    : item.category || "";
                  const creatorLabel = Array.isArray(item.creator)
                    ? item.creator[0]
                    : item.creator || "";
                  return (
                    <article
                      key={item.article_id || item.link}
                      className="group overflow-hidden rounded-2xl border border-slate-800/80 bg-gradient-to-br from-slate-950 via-slate-900/60 to-slate-950 shadow-[0_18px_40px_-30px_rgba(56,189,248,0.45)] transition hover:-translate-y-0.5 hover:border-cyan-500/40"
                    >
                      <div className="grid gap-0 md:grid-cols-[280px_1fr]">
                        <div className="relative h-64 md:h-full">
                          {imageUrl ? (
                            ...
                            <div className="h-64 md:h-full bg-slate-900/60 flex items-center justify-center text-sm text-slate-500">
                              No image available
                            </div>
                          )}
                          <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-900/30 to-transparent" />
                          <div className="absolute top-3 left-3 flex flex-wrap items-center gap-2">
                            <span className="rounded-full bg-slate-950/70 border border-slate-700 px-2.5 py-1 text-[11px] text-slate-200">
                              {sourceName}
                            </span>
                            {categoryLabel && (
                              <span className="rounded-full bg-cyan-500/10 border border-cyan-500/30 px-2.5 py-1 text-[11px] text-cyan-200">
                                {categoryLabel}
                              </span>
                            )}
                          </div>
                          {articleTime && (
                            <div className="absolute bottom-3 left-3 text-[11px] text-slate-200/90 bg-slate-950/70 border border-slate-700 rounded-full px-2.5 py-1">
                              {new Date(articleTime).toLocaleString()}
                            </div>
                          )}
                        </div>
                        <div className="p-4 sm:p-5 flex flex-col gap-3">
                          ...
                          <h3 className="text-lg sm:text-xl font-semibold leading-snug text-slate-100 group-hover:text-cyan-200 transition">
                            {item.title}
                          </h3>
                          {item.description && (
                          ...
                          <div className="mt-auto flex flex-wrap items-center gap-3">
                            {creatorLabel && (
                              <div className="text-[11px] text-slate-400">
                                By <span className="text-slate-200">{creatorLabel}</span>
                              </div>
                            )}
                            {item.link && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="ml-auto inline-flex items-center gap-2 rounded-full bg-cyan-500/10 border border-cyan-400/30 px-3 py-1.5 text-sm text-cyan-200 hover:bg-cyan-500/20 transition"
                              >
                                Read more
                              </a>
                            )}
        ...
        {activeTab === "posts" && (
          <section className="space-y-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-400 mt-1">
                All recent posts from the community.
              </p>
            </div>
            ...
            {!postsLoading && postsLoaded && posts.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-800 bg-slate-950/40 p-8 text-center text-sm text-slate-400">
                No posts yet.
              </div>
            ) : (
              <div className="space-y-4">
                {posts.map((post) => {
                  ...
                  const sortedComments = sortCommentThreads(
                    comments,
                    commentSort
                  );
                  ...
                  const displayName = post.username || "User";
                  const displayAvatarUrl = resolveMediaUrl(post.avatar);
                  const isPostOwner =
                    currentUserId &&
                          ...
                          {[
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                const existing = likeDebounceRef.current.get(
                                  post.id
                                );
                                if (existing?.timeoutId) {
                                  clearTimeout(existing.timeoutId);
